    ...
    <div className="space-y-8 animate-fade-in pb-16">
      <div className="px-4 flex justify-between items-end">
        ...
        <div className="hidden md:block bg-primary-50 px-4 py-2 rounded-2xl border border-primary-100">
           <p className="text-[10px] font-black text-primary-600 uppercase">Status do Sistema</p>
           <p className="text-xs font-bold text-slate-600 flex items-center gap-1"><Clock size={12}/> Sincronizado agora</p>
        </div>
      </div>

      <div className="grid grid-cols-2 md:grid-cols-4 gap-6 px-4">
        <StatsCard title="Receita (Total)" value={formatCurrency(summary.totalRevenue)} icon={ArrowUpRight} color="blue" />
        ...
        <StatsCard title="Lucro Projetado" value={formatCurrency(summary.netProfit)} icon={DollarSign} color="green" trend={`${summary.profitMargin.toFixed(1)}% margem`} />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 px-4">
        <div className="bg-white p-10 rounded-[3.5rem] border shadow-sm flex flex-col justify-between">
          <div>
            <h3 className="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-8 flex items-center gap-2">
              <BarChart3 size={18}/> Performance Financeira
            </h3>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={[
                  ...
                  { name: 'Comissão', valor: summary.totalCommissions },
        ...
        <div className="bg-slate-950 p-12 rounded-[4rem] text-white flex flex-col justify-center relative overflow-hidden shadow-2xl">
          ...
          <div className="relative z-10">
            <h3 className="text-3xl font-black uppercase tracking-tighter mb-4 leading-none">Eficiência Operacional</h3>
            <p className="text-slate-500 text-xs font-bold uppercase tracking-widest mb-12">Aproveitamento real da frota</p>
            
            <div className="space-y-8">
              <div className="flex justify-between items-end border-b border-white/5 pb-8">
                 <div>
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Margem Líquida</p>
                   <p className="text-5xl font-black text-emerald-400 mt-2">{summary.profitMargin.toFixed(1)}%</p>
                 </div>
                 <div className="text-right">
                   <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Média p/ Viagem</p>
              ...
              <div className="grid grid-cols-2 gap-6">
                 <div className="bg-white/5 p-6 rounded-3xl border border-white/5">
                    <p className="text-[10px] font-black text-rose-400 uppercase mb-2">Total Despesas</p>
                    <p className="text-2xl font-black">{formatCurrency(summary.totalTripExpenses + summary.totalFixedExpenses)}</p>
                 </div>
                 <div className="bg-white/5 p-6 rounded-3xl border border-white/10">
                    <p className="text-[10px] font-black text-amber-400 uppercase mb-2">Comissões</p>
