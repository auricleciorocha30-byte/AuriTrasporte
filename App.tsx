...
const sanitizeExpensePayload = (payload: any) => {
  const allowedKeys = ['trip_id', 'vehicle_id', 'description', 'amount', 'category', 'date', 'user_id', 'is_paid', 'due_date', 'installments_total', 'installments_remaining'];
  const sanitized: any = {};
      ...
      <div className="w-full max-w-md bg-white rounded-[3rem] shadow-2xl p-10 animate-fade-in border border-white/10">
        ...
        <form onSubmit={handleAuth} className="space-y-5">
          <div className="space-y-1.5">
            ...
            <div className="relative">
              ...
              <input 
                ...
                className="w-full pl-12 pr-4 py-4.5 bg-slate-50 border border-slate-100 rounded-2xl font-bold outline-none focus:ring-2 focus:ring-primary-500 transition-all placeholder:text-slate-300" 
                value={email} 
          ...
          <button 
            ...
            type="submit" 
            className="w-full py-5 bg-primary-600 text-white rounded-2xl font-black text-lg shadow-xl hover:bg-primary-700 active:scale-95 transition-all flex items-center justify-center gap-3"
          >
        ...
        <div className="mt-8 pt-6 border-t border-slate-50 text-center">
          <button onClick={() => {setIsSignUp(!isSignUp); setError('');}} className="text-primary-600 font-black text-sm uppercase hover:underline tracking-tight">
    ...
    <div className="flex h-screen bg-slate-50 overflow-hidden">
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        <header className="h-16 bg-white border-b flex items-center justify-between px-6 shrink-0 z-10" style={{ paddingTop: 'env(safe-area-inset-top)', height: 'calc(4rem + env(safe-area-inset-top))' }}>
          ...
          <div className="p-2 text-slate-500 hover:bg-slate-50 rounded-full transition-colors cursor-pointer"><Bell size={22} /></div>
        </header>
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
          {currentView === AppView.EXPENSES && (
            <ExpenseManager 
              ...
              onDeleteExpense={async (id) => { if(window.confirm('Excluir este lançamento?')) { await supabase.from('expenses').delete().eq('id', id); fetchData(); } }} 
              isSaving={isSaving} 
            />
          )}
          {currentView === AppView.TRIPS && (
            <TripManager 
              ...
              onUpdateTrip={async (id, t) => { await supabase.from('trips').update(t).eq('id', id); fetchData(); }}
              onUpdateStatus={async (id, s, km) => { 
                await supabase.from('trips').update({status: s}).eq('id', id); 
                if(km) {
                  const trip = trips.find(x => x.id === id);
                  if(trip?.vehicle_id) await supabase.from('vehicles').update({current_km: km}).eq('id', trip.vehicle_id);
                }
                fetchData(); 
              }}
              onDeleteTrip={async (id) => { if(window.confirm('Excluir esta viagem?')) { await supabase.from('trips').delete().eq('id', id); fetchData(); } }}
              isSaving={isSaving}
            />
          )}
          {currentView === AppView.VEHICLES && (
            <VehicleManager 
              vehicles={vehicles}
              onAddVehicle={async (v) => { await supabase.from('vehicles').insert([{...v, user_id: session.user.id}]); fetchData(); }}
              onUpdateVehicle={async (id, v) => { await supabase.from('vehicles').update(v).eq('id', id); fetchData(); }}
              onDeleteVehicle={async (id) => { if(window.confirm('Excluir este veículo?')) { await supabase.from('vehicles').delete().eq('id', id); fetchData(); } }}
              isSaving={isSaving}
            />
          )}
          {currentView === AppView.MAINTENANCE && (
            <MaintenanceManager 
              ...
              vehicles={vehicles}
              onAddMaintenance={async (m) => { await supabase.from('maintenance').insert([{...m, user_id: session.user.id}]); fetchData(); }}
              onDeleteMaintenance={async (id) => { if(window.confirm('Excluir este registro?')) { await supabase.from('maintenance').delete().eq('id', id); fetchData(); } }}
              isSaving={isSaving}
            />
          )}
